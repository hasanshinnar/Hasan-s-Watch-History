...
def video_data_extract():
    ...
    with open(watch_history_file, "r", encoding="utf-8") as WatchHistory:
        data = json.load(WatchHistory)
        video_ids = []
        video_data = []
        for item in data:
            ...
            with open("video_ids.txt", "w") as WatchHistory:
                for vid in video_ids:
                    WatchHistory.write(f"{vid}\n")
        for item in data:
            url = item.get("titleUrl", "")
            time = item.get("time", "")
